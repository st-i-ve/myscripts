// ==UserScript==
// @name         Sequential Section Navigator V7 with Double Knowledge Execution
// @namespace    http://tampermonkey.net/
// @version      7.0
// @description  Navigate through noOutline sections with double execution for knowledge blocks
// @match        *://*/*
// @grant        none
// ==/UserScript==
...
(function () {
  ...
  function updateCategoryDisplay(category, index, total, executed = false, customStatus = "") {
    ...
    if (customStatus) {
      ...
      status = isKnowledgeBlock(category) ? " - EXECUTED! (2x)" : " - EXECUTED!";
    }
  ...
  async function navigateToNextSection() {
    ...
    if (currentElement) {
      ...
      // execute the block function (with double execution for knowledge blocks)
