// ==UserScript==
// @name         Sequential Section Navigator V9 - SkillsLine with Scenario Support
// @namespace    http://tampermonkey.net/
// @version      9.0
// @description  Navigate through noOutline sections on skillsline.com with scenario and quiz condition support
...
(function () {
  ...
  function identifyBlockType(element) {
    // check for scenario activity
    if (element.querySelector('.block-scenario')) {
      return "scenario activity - auto continue";
    }
  ...
  function isInteractiveBlock(category) {
    const interactiveTypes = [
      "scenario activity - auto continue",
  ...
  function isManualBlock(category) {
    return category === "sorting activity - manual sorting" || category === "scenario activity - auto continue";
  }

  // function to check quiz condition that should stop the script
  function checkQuizCondition() {
    ...
    try {
      ...
      // i look for quiz elements that are visible
      const quizElements = iframeDoc.querySelectorAll('.quiz');
      
      for (const quiz of quizElements) {
        // check if quiz is visible
        const rect = quiz.getBoundingClientRect();
        const isVisible = rect.top >= 0 && rect.bottom <= iframeDoc.documentElement.clientHeight;
        
        if (isVisible) {
          // check if quiz is NOT inside a noOutline element
          const isInsideNoOutline = quiz.closest('.noOutline') !== null;
          
          if (!isInsideNoOutline) {
            // check if there are NO noOutline elements currently visible
            const visibleNoOutlines = Array.from(iframeDoc.querySelectorAll('.noOutline')).filter(el => {
              const elRect = el.getBoundingClientRect();
              return elRect.top >= 0 && elRect.bottom <= iframeDoc.documentElement.clientHeight;
            });
            
            if (visibleNoOutlines.length === 0) {
              console.log("🛑 Quiz condition met: quiz visible, not in noOutline, no noOutlines visible");
              return true;
            }
          }
        }
      ...
      console.warn("❌ Error checking quiz condition:", err);
      return false;
    }
  }

  // function to stop script and hide buttons
  function stopScriptAndHideButtons() {
    console.log("🛑 Stopping script and hiding buttons due to quiz condition");
    ...
    console.log("✅ Script stopped and buttons hidden");
  }

  // execution function for scenario activities
  function executeScenario(element, iframeDoc) {
    console.log("🎭 Executing scenario activity logic - auto continue buttons");
    ...
    // i check each selector and click buttons if visible
    SCENARIO_SELECTORS.forEach(selector => {
      const buttons = element.querySelectorAll(selector);
      
      buttons.forEach(button => {
        if (button && button.offsetParent !== null) {
          console.log(`🎭 Clicking scenario button 15 times: ${selector}`);
    ...
    console.log(`✅ Executed scenario logic on ${totalClicked} buttons`);
  ...
  function waitForNextButton(element, category) {
    return new Promise((resolve) => {
      ...
      let statusMessage = "";
      if (category === "scenario activity - auto continue") {
        statusMessage = "AUTO CONTINUE EXECUTED - Waiting for next button";
      } else {
        statusMessage = "MANUAL SORTING REQUIRED - Waiting for next button";
      }
      
      updateCategoryDisplay(category, currentIndex, noOutlineElements.length, false, statusMessage);
  ...
  async function executeBlockFunction(element, category) {
    ...
    try {
      ...
      switch (category) {
        case "scenario activity - auto continue":
          executed = executeScenario(element, iframeDoc);
          if (executed) {
            // i wait for next button to appear after scenario execution
  ...
  function updateCategoryDisplay(category, index, total, executed = false, customStatus = "") {
    ...
    if (customStatus) {
      ...
      if (isKnowledgeBlock(category)) {
        status = " - EXECUTED! (2x)";
      } else if (category === "sorting activity - manual sorting") {
        status = " - HINTS APPLIED!";
      } else if (category === "scenario activity - auto continue") {
        status = " - AUTO CONTINUE EXECUTED!";
      } else {
        status = " - EXECUTED!";
      }
    }
  ...
  async function navigateToNextSection() {
    ...
    // i check quiz condition first - if met, stop script and hide buttons
    if (checkQuizCondition()) {
      stopScriptAndHideButtons();
      return;
    }
    ...
    if (currentElement) {
      ...
      // i add the 6-second wait as requested when quiz condition is not met
      delay += 6000;
  ...
  console.log("✅ Sequential Section Navigator V9 loaded for SkillsLine.com");
})();