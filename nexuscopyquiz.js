...
(function() {
    ...
    function copyQuestionAndChoices() {
      const questionSection = document.querySelector('.question-section:not(.hidden)');
      if (!questionSection) {
        console.log("No visible question section found!");
        return;
      }
  
      const questionContent = questionSection.querySelector('.question-content p');
      const questionText = questionContent ? questionContent.innerText.trim() : '';
  
      const choiceLabels = questionSection.querySelectorAll('.question-choice label.label');
      const choices = Array.from(choiceLabels).map(label => label.innerText.trim());
