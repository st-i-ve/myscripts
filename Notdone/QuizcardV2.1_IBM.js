// ==UserScript==
// @name         AI Quiz - Section-Based Answer Selector V2.1 (With Fallbacks)
// @namespace    http://tampermonkey.net/
// @version      2.1
// @description  Auto-selects correct answer, submits, and clicks NEXT based on section heading, with fallback mechanisms for different HTML structures
// @match        *://*/*
// @grant        none
// ==/UserScript==
...
(function () {
  ...
  // i added this function to get quiz options with fallback mechanisms
  const getQuizOptions = (container) => {
    ...
    if (options.length === 0) {
      // fallback to input type radio with specific class
  ...
  // i added this function to extract answer text with fallback mechanisms
  const getAnswerText = (option, doc) => {
    ...
    // fallback to quiz-multiple-choice-option__text divs
    if (!answerText) {
      console.log("🔄 Falling back to .quiz-multiple-choice-option__text selection");
      // i look for the closest parent label or container and find the text div
  ...
  setInterval(() => {
    ...
    try {
      ...
      // i use the new function with fallback mechanisms
      ...
      options.forEach((option) => {
        // i use the new function to get answer text with fallbacks
