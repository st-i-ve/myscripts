// ==UserScript==
// @name         AI Quiz - Section-Based Answer Selector V2.2 (Enhanced Fallbacks)
// @namespace    http://tampermonkey.net/
// @version      2.2
// @description  Auto-selects correct answer, submits, and clicks NEXT based on section heading, with enhanced fallback mechanisms for radio/checkbox inputs and answer text extraction
// @match        *://*/*
// @grant        none
// ==/UserScript==
...
(function () {
  ...
  const getQuizOptions = (container) => {
    ...
    if (options.length === 0) {
      ...
      options = container.querySelectorAll('input[type="radio"].quiz-multiple-choice-option__input');
    }
    
    if (options.length === 0) {
      // fallback 2: multiple-choice checkboxes (single-choice style)
      console.log("🔄 Falling back to input[type='checkbox'] single-choice selection");
      options = container.querySelectorAll('input[type="checkbox"].quiz-multiple-choice-option__input');
    }
    
    if (options.length === 0) {
      // fallback 3: multiple-choice checkboxes (multiple-response style)
      console.log("🔄 Falling back to input[type='checkbox'] multiple-response selection");
      options = container.querySelectorAll('input[type="checkbox"].quiz-multiple-response-option__input');
    }
  ...
  const getAnswerText = (option, doc) => {
    ...
    if (!answerText) {
      console.log("🔄 Falling back to .quiz-multiple-choice-option__text selection");
      const parentLabel = option.closest('label') || option.closest('.quiz-multiple-choice-option');
      if (parentLabel) {
        const textDiv = parentLabel.querySelector('.quiz-multiple-choice-option__text');
        if (textDiv) {
    ...
    if (!answerText) {
      console.log("🔄 Falling back to .quiz-multiple-response-option__text selection");
      const parentLabel = option.closest('label') || option.closest('.quiz-multiple-response-option');
      if (parentLabel) {
        const textDiv = parentLabel.querySelector('.quiz-multiple-response-option__text');
        if (textDiv) {
          answerText = textDiv.innerText.trim();
        }
      }
